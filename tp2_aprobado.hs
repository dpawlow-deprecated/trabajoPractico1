...
{-ejemplo1' = (O P P)  `Imp` (No (Y Q R)) -- (P v P) => ~(Q ^ R)-}
{-ejemplo2' = No (Y (Imp R P) (No (O Q R))) -- ~ ((R => P) ^ ~(Q v R))-}
{-ejemplo3' = No (No (Y (Imp R P) (No (O Q R)))) -- ~~((R=>P) ^ ~(QvR))-}
{-ejemplo4' = No (Y (Imp R P) (No (No (O Q R)))) -- ~((R=>P) ^ ~~(RvQ))-}
{-ejemplo5' = ejemplo1' `Imp` ejemplo4'-- ((P v P) => ~(Q ^ R)) => ~((R=>P) ^ ~~(RvQ))-}


--Ariel Cuellar (184/91, No Inscripto) Miércoles Mañana.
--Dante Pawlow (449/12) Miércoles Mañana.
...
--CORR: funciona bien, codigo correcto
...
--Corr: funciona correctamente. Codigo ok
...
--Corr: funciona perfecto. 

-- Ejercicio 7:
data TipoFormula = Tautologia | Contradiccion | Contingencia deriving (Show, Eq)
...
tests prop (-1) bool = True
tests prop n    bool = (bool == evaluar prop (combinacion n)) && tests prop (n-1) bool
